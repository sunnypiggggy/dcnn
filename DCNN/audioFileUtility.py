...
def LoadAudioBatch(classNum,numPerClass):
    soundList=[]
    rootPath=os.getcwd()
    print(rootPath)
    f= os.path.join(rootPath,'ESC-50-master')
    #classDirNames=os.listdir(f)
    ...
    for subDirName in classDirNames:
        ...
        for xx in range(len(filePath)):
            print(os.path.join(fileRootPath,filePath[xx]))
            soundList.append((subDirName,LoadAudioFiles(os.path.join(fileRootPath,filePath[xx]))))        
    print(len(soundList))
